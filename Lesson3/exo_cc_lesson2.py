...
def get_prices_from_url(url):
    htmlContent = req.get(url).content
    tree = html.fromstring(htmlContent)
    articles = {}
    article_tables = tree.xpath("//div[starts-with(@class,'pricing-table pricing-table-')]")
    for art in article_tables:
        name = art.find_class('pricing-table-header')[0].find('h2').text_content()
        ul = art.find_class('pricing-table-list')[0]
        price = art.find_class('pricing-table-price')[0].text.strip().replace("<.*$", '')
        lis = ul.findall('li')

        props = {'price': price,
                 'storage': lis[3].text_content().strip().split(' ')[0],
                 'databases': int(lis[4].text_content().strip().split(' ')[0])}
        articles[name] = props
    return articles
...
def extract_beer_list_infos(url):
    beersIn = json.loads(req.get(url).content)
    beersOut = []
    for beer in beersIn:
        print(beer)
        beersOut.append({'name': beer.title, 'price': beer.displayInformationPrice.price, 'volume': beer.volume, 'note': 1, 'ebc': beer.ebc})
    return beersOut
